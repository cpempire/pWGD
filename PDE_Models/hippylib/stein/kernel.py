...
from .metric import MetricPrior, MetricPosteriorSeparate, MetricPosteriorAverage, MetricPosteriorAverageReg
...
from .BM_utils import calculate_BM_ibw
...
class Kernel:
    ...
    def __init__(self, model, particle, variation, options, comm):
        ...
        elif self.type_metric is 'posterior_average_reg':
            self.metric = MetricPosteriorAverageReg(model, particle, variation, options)
        else:
    ...
    def update(self, particle, variation):
        ...
        for n in range(particle.number_particles_all):
            ...
            if self.type_scaling == 1:
                ...
                if self.batch_proj:
                    self.scale[n] = len(self.group_idx)
                else:
                ...
                self.scale[n] = np.median(self.value_set[n]) / np.maximum(1, 0.5*np.log(self.nproc*particle.number_particles+1))
        ...
        if self.type_scaling == 5:
            self.scale[:] = np.median(self.value_set)/ np.maximum(1, np.log(self.nproc*particle.number_particles))
        if self.type_scaling == 6:
            if self.it == 1:
                self.scale[:] = np.median(self.value_set)/ np.maximum(1, np.log(self.nproc*particle.number_particles))

            particle_local = np.concatenate([p.get_local().reshape([-1,1]) for p in self.particle.particles],1)
            if self.batch_proj:
                particle_local = particle_local[self.group_idx, :]
            BM_ibw = calculate_BM_ibw(particle_local, 1./self.scale[0], self.alpha)
            self.scale[:] = 1./BM_ibw
