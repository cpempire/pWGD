...
class GradientDescent:
    ...
    def solve_batch(self, batchsize=5):
        # use gradient decent method to solve the optimization problem
        rel_tol = self.options["rel_tolerance"]
        abs_tol = self.options["abs_tolerance"]
        max_iter = self.options["max_iter"]
        ...
        # set tolerance for gradient norm and step norm
        for m in range(self.particle.number_particles):  # solve for each particle
            # evaluate gradient
            gradient = self.particle.generate_vector()
            if self.WGF:
                self.gradient_norm_init[m] = self.gradientSeparated_WGF(gradient, m)
            else:
                self.gradient_norm_init[m] = self.gradientSeparated(gradient, m)
            self.tol_gradient[m] = max(abs_tol, self.gradient_norm_init[m] * rel_tol)

        self.alpha = self.search_size * np.ones(self.particle.number_particles_all)
        self.step_norm_init = self.gradient_norm_init * self.alpha
        ...
        while self.it < max_iter and (self.converged is False):
            ...
            for j in range(num_group):
                if j<num_group-1:
                    group_idx = idx_all[j*batchsize:(j+1)*batchsize]
                else:
                    group_idx = idx_all[j*batchsize:]
                ...
                for m in range(self.particle.number_particles):  # solve for each particle
                    ...
                    phat[m].axpy(-1.0, gradient)
                    self.pg_phat[m] = gradient.inner(phat[m])
                    # print("j = ", j, " group_idx = ", group_idx, " self.pg_phat[m] = ", self.pg_phat[m])
                ...
                if j == 0:
                    ...
                    self.step_norm = np.max(np.array([self.step_norm, self.pstep_norm * self.alpha]), axis=0)

                # move all particles in the new directions, pm = pm + self.alpha[m] * sum_n phat[n] * k(pn, pm)
                self.particle.move(self.alpha, pstep)
                self.variation.update(self.particle, batch=True)

            # compute gradient norm after one loop of num_group
            self.kernel.update(self.particle, self.variation)
            for m in range(self.particle.number_particles):  # solve for each particle
                # evaluate gradient
                gradient = self.particle.generate_vector()
    ...
    def solve_adam(self):
        ...
        while self.it < max_iter and (self.converged is False):
            ...
            for m in range(self.particle.number_particles):  # solve for each particle
                ...
                if self.WGF:
                    gradient_norm[m] = self.gradientSeparated_WGF(gradient, m)
                else:
