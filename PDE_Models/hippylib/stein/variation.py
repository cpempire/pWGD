...
class Variation:
    ...
    def update(self, particle, iter=0, step_norm=1, batch=False):
        self.particle = particle
        ...
        if self.options["is_projection"]:
            ...
            self.is_bases_updated = False
            if ((iter == 0) or (step_norm < self.options["step_projection_tolerance"])) and not batch:  # replace iter by (np.mod(iter, self.options["update_step"])
