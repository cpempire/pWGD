...
labelTrue = ['pWGD d=81', 'pWGD d=289', 'pWGD d=1089', 'pWGD d=4225']
labelFalse = ['WGD d=81', 'WGD d=289', 'WGD d=1089', 'WGD d=4225']
fig1 = plt.figure(1)
fig2 = plt.figure(2)
for case in [0, 1, 2, 3]:
    ...
    for i in range(Ntrial):
        filename = "accuracy-" + str(d) + 'x' + str(d) + "/data/data_nDimensions_"+str((d+1)**2)+"_nCores_"+str(16)+"_nSamples_" + str(N) + "_isProjection_" + str(True) + "_WGF_batch5_" + str(i+1) + ".p"
...
filename = "figure/error_mean_WGD.pdf"
fig1.savefig(filename, format='pdf', bbox_inches='tight')
filename = "figure/error_mean_WGD.eps"
fig1.savefig(filename, format='eps', bbox_inches='tight')
...
filename = "figure/error_variance_WGD.pdf"
fig2.savefig(filename, format='pdf', bbox_inches='tight')
filename = "figure/error_variance_WGD.eps"
fig2.savefig(filename, format='eps', bbox_inches='tight')
...
fig1 = plt.figure(1)
for i in range(Ntrial):
    plt.plot(dPara, np.log10(meanErrorDimensionFalse[:, i]), makerFalse[case], alpha=0.2)
    plt.plot(dPara, np.log10(meanErrorDimensionTrue[:, i]), makerTrue[case], alpha=0.2)
    plt.plot(dPara, np.log10(meanErrorDimensionTrueBatch[:, i]), makerTrueBatch[case], alpha=0.2)

plt.plot(dPara, np.log10(np.sqrt(np.mean(meanErrorDimensionFalse ** 2, axis=1))), makerRMSEFalse[case], linewidth=2,
         label='WGD')
plt.plot(dPara, np.log10(np.sqrt(np.mean(meanErrorDimensionTrue ** 2, axis=1))), makerRMSETrue[case], linewidth=2,
         label='pWGD')
plt.plot(dPara, np.log10(np.sqrt(np.mean(meanErrorDimensionTrueBatch ** 2, axis=1))), makerRMSETrueBatch[case], linewidth=2,
         label='pWGD-batch')
plt.figure(1)
plt.legend(fontsize=16)
...
filename = "figure/error_mean_dimension_WGD.pdf"
fig1.savefig(filename, format='pdf', bbox_inches='tight')
filename = "figure/error_mean_dimension_WGD.eps"
fig1.savefig(filename, format='eps', bbox_inches='tight')
...
fig2 = plt.figure(2)
for i in range(Ntrial):
    plt.plot(dPara, np.log10(varianceErrorDimensionFalse[:, i]), makerFalse[case], alpha=0.2)
    plt.plot(dPara, np.log10(varianceErrorDimensionTrue[:, i]), makerTrue[case], alpha=0.2)
    plt.plot(dPara, np.log10(varianceErrorDimensionTrueBatch[:, i]), makerTrueBatch[case], alpha=0.2)

plt.plot(dPara, np.log10(np.sqrt(np.mean(varianceErrorDimensionFalse ** 2, axis=1))), makerRMSEFalse[case], linewidth=2,
         label='WGD')
plt.plot(dPara, np.log10(np.sqrt(np.mean(varianceErrorDimensionTrue ** 2, axis=1))), makerRMSETrue[case], linewidth=2,
         label='pWGD')
plt.plot(dPara, np.log10(np.sqrt(np.mean(varianceErrorDimensionTrueBatch ** 2, axis=1))), makerRMSETrueBatch[case], linewidth=2,
         label='pWGD-batch')
plt.legend(fontsize=16)
plt.xlabel("dimension", fontsize=16)
plt.ylabel("$\log_{10}$(RMSE of variance)", fontsize=16)
...
filename = "figure/error_variance_dimension_WGD.pdf"
fig2.savefig(filename, format='pdf', bbox_inches='tight')
filename = "figure/error_variance_dimension_WGD.eps"
fig2.savefig(filename, format='eps', bbox_inches='tight')
