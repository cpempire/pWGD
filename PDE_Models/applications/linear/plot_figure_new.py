...
WGF = {}
WGF['meanErrorL2norm'] = np.zeros((4, Ntrial, Niter))
WGF['varianceErrorL2norm'] = np.zeros((4, Ntrial, Niter))
WGF['label'] = 'WGF d=17'
WGF['filename'] = './data/data_nDimensions_{}_nCores_1_nSamples_{}_isProjection_False_WGF'.format(d,N)

pWGF = {}
pWGF['meanErrorL2norm'] = np.zeros((4, Ntrial, Niter))
pWGF['varianceErrorL2norm'] = np.zeros((4, Ntrial, Niter))
pWGF['label'] = 'pWGF d=17'
pWGF['filename'] = './data/data_nDimensions_{}_nCores_1_nSamples_{}_isProjection_True_WGF'.format(d,N)

pWGF_post = {}
pWGF_post['meanErrorL2norm'] = np.zeros((4, Ntrial, Niter))
pWGF_post['varianceErrorL2norm'] = np.zeros((4, Ntrial, Niter))
pWGF['label'] = 'pWGF-post d=17'
...
for (j, algo) in enumerate(algorithms):
    ...
    plt.plot(np.log10(np.sqrt(np.mean(algo['meanErrorL2norm'][i, :]**2, axis=0))), colors[j], linewidth=3)
    plt.plot(iters[0:iter_num:interval],np.log10(np.sqrt(np.mean(algo['meanErrorL2norm'][i, :]**2, axis=0)))[0:iter_num:interval], colors[j]+markers[j], linewidth=3, label=algo['label'])
...
for (j, algo) in enumerate(algorithms):
    ...
    plt.plot(np.log10(np.sqrt(np.mean(algo['varianceErrorL2norm'][i, :]**2, axis=0))), colors[j], linewidth=3)
    plt.plot(iters[0:iter_num:interval],np.log10(np.sqrt(np.mean(algo['varianceErrorL2norm'][i, :]**2, axis=0)))[0:iter_num:interval], colors[j]+markers[j], linewidth=3, label=algo['label'])
