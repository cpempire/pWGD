...
class DILI:
    ...
    """
    def __init__(self,parameter_init,geometry_fun,step_size,proposal='LI_Langevin',adpt_h=False,**kwargs):
        """
        ...
        self.loglik,self.gradlik,_,self.eigs=self.geom(self.v,[0,1,1.5,2], k=100)
...
#             h_adpt['h']=self._init_h()
    ...
    def update_LIS(self,threshold_l=.01,threshold_s=1e-4,threshold_g=.01):
        ...
        _,_,_,eigs=self.geom(self.v,geom_ord=[1.5])
...
#         Theta_r1r=self.Theta_r.T.dot(Theta_r)
        self.Theta_r1r=self.Theta_r.T.dot(Theta_r)
...
#             v_ref=np.zeros_like(self.v)
        ...
        if self.proposal is 'LI_prior':
            ...
            v+=self.G.dot(self.gradlik)
            # v=self.model.prior.gen_vector(v)
            loglik,gradlik,_,_=self.geom(v,geom_ord=[0,1])
            Psi_r_v=self.Psi_r.T.dot(self.v); Psi_r_v1=self.Psi_r.T.dot(v)
...
#         self.d_F=np.linalg.norm(np.log(w.real[w.real>0]))
    ...
    def save_samp(self):
        ...
        res2save=[self.h,self.proposal,self.samp,self.logLik,self.acpt,self.time,self.times,
