...
class HessianSeparated:
    ...
    def init_vector(self, pvec, dim):
        if self.options["is_projection"]:
            pvec.init(self.particle.dimension)
        else:
    ...
    def assemble(self):
        ...
        hess = np.zeros((self.particle.dimension, self.particle.dimension))

        if self.kernel.delta_kernel:
            hess += self.variation.hessian_misfit_gather[self.rank][self.m] + np.eye(self.particle.dimension)
            ...
            for p in range(self.nproc):
                for ie in range(self.particle.number_particles):  # take the expectation
                    hess += kernel_value[p][ie] ** 2 * (self.variation.hessian_misfit_gather[p][ie]
                                                        + np.eye(self.particle.dimension))
    ...
    def solve(self):
        ...
        while self.it < max_iter and (self.converged is False):
            ...
            for m in range(self.particle.number_particles_all):  # solve for each particle used for constructing the map
                ...
                if self.options["is_projection"] or \
                    (self.options["type_parameter"] is "vector" and not self.options["low_rank_Hessian"]):  # direct solve
            ...
            for m in range(self.particle.number_particles_all):
                ...
                # restrict the update to the parameter range
                if hasattr(self.model.prior, "ranges"):
                    if self.options["is_projection"]:
                        self.model.prior.restrict(self.particle.coefficients[m], pstep[m])
                    else:
                        self.model.prior.restrict(self.particle.particles[m], pstep[m])

                if self.options["is_projection"]:
                    self.pstep_norm[m] = np.sqrt(pstep[m].inner(pstep[m]))/self.particle.dimension
                    pstep_m = pstep[m].get_local()
                    for r in range(self.particle.dimension):
