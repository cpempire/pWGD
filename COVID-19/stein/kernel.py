...
class Kernel:
    ...
    def communication(self):
        ...
        if self.options["type_optimization"] is not "gradientDescent":
                ...
                for n in range(self.particle.number_particles_all):
                    for p in range(self.nproc):
                        for m in range(self.particle.number_particles_all):
                            gradient_set_array[n, p, m, :] = self.gradient_set[n][p][m]
            ...
            if self.save_kernel:
                ...
                for p in range(self.nproc):
                    for n in range(self.particle.number_particles_all):
                        gradient_sum_array[p, n, :] = self.gradient_sum[p][n]
                ...
                for p in range(self.nproc):
                    for n in range(self.particle.number_particles_all):
                        self.gradient_sum_reduce[p][n][:] = gradient_sum_reduce_array[p, n, :]
        ...
        for n in range(particle.number_particles_all):
            if self.options["is_projection"]:
                if self.options["kernel_vectorized"]:
                    pn = particle.coefficients_array[n]
                    pm = particle.coefficients_gather_array
                    pdiff = np.subtract(pn, pm)
                    # self.value_set[n, :, :] = np.sum(np.multiply(pdiff, pdiff), axis=2)  # todo include metric matrix !!
                    pdiffM = np.einsum('ijk, kl', pdiff, self.metric.matrix)
                    self.value_set[n, :, :] = np.sum(np.multiply(pdiffM, pdiff), axis=2)
                    ...
                    for p in range(self.nproc):
                        for m in range(particle.number_particles_all):
                            ...
                            self.value_set[n, p, m] = phelp.dot(pout)  # (pn-pm)^T * M * (pn-pm)
                            grad_norm_set[n, p, m] = np.sqrt(pout.dot(pout))
                            if self.save_kernel:
                                self.gradient_set[n][p][m][:] = 0.
                                self.gradient_set[n][p][m] += 1.0 * pout  # M * (pn-pm)
            else:
                if self.options["kernel_vectorized"]:
                    ...
                    for p in range(self.nproc):
                        for m in range(particle.number_particles_all):
                            ...
                            phelp += 1.0 * pn
                            phelp -= 1.0 * pm
                            pout = self.model.generate_vector()
                            self.metric.mult(phelp, pout, n)
                            self.value_set[n, p, m] = phelp.dot(pout)  # (pn-pm)^T * M * (pn-pm)
                            phelp[:] = 0.
                            self.model.prior.Msolver.solve(phelp, pout)
                            grad_norm_set[n, p, m] = np.sqrt(pout.dot(phelp))
                            if self.save_kernel:
                                self.gradient_set[n][p][m][:] = 0.
                                self.gradient_set[n][p][m] += 1.0 * pout  # M * (pn-pm)
        ...
        for n in range(particle.number_particles_all):
            ...
            if self.save_kernel:
                if self.options["kernel_vectorized"]:
                    ...
                    for p in range(self.nproc):
                        for m in range(particle.number_particles_all):
                            self.gradient_set[n][p][m][:] = gradient_set[n][p][m]
                else:
                    for p in range(self.nproc):
                        for m in range(particle.number_particles_all):
                            # # evaluate the kernel k(pn,pm) = exp(-(pn-pm)^T * M * (pn-pm))
            ...
            for p in range(self.nproc):
                for m in range(particle.number_particles_all):
                    ...
                    for n in range(particle.number_particles):
                        ...
                        self.gradient_sum[p][m] += 1.0 * self.gradient_set[n][p][m]
    ...
    def gradient(self, n=0, p=0, m=0):
        ...
        gradient += value/self.scale[n] + ghelp
        ...
        for p in range(self.nproc):
            for m in range(self.particle.number_particles_all):
                ...
                if (p != self.rank or m != n) and self.delta_kernel:
                    ...
                    gradients[p][m] += 1.0 * self.gradient(n=n, p=p, m=m)
