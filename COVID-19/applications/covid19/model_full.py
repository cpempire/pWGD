...
class Misfit():
    ...
    def forward(self, x):
        ...
        # u = np.append(np.log(solution[self.loc, 5]), np.log(np.diff(solution[self.loc, 7])))
...
if model_type is "scalar":
    ...
    filename_prex = "figure/initial_scalar_" + state_name + "_full_"
    savefilename = "data/initial_scalar_solution_" + state_name + "_full"
    ...
    number_control_change_times = 60
...
prior = Laplacian(misfit.dimension)
...
if __name__ ==  "__main__":
    ...
    lower_bound = [0.0, 0.1, 0.8]
    upper_bound = [0.95, 0.1, 0.8]

    for i_p in range(3, len(misfit.controls)):
    ...
    np.savez(savefilename, configurations=misfit.configurations, parameters=misfit.parameters,
             simulation_first_confirmed=misfit.simulation_first_confirmed, today=today,
             solution=solution, controls=misfit.controls,
             solution_opt=solution_opt, controls_opt=controls_opt, parameters_opt=controls_opt[1:])
